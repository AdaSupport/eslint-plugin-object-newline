const fixer = (context, nestTab, node, spacer = "\n") => (eslintFixer) => {
  ...
  const mapper = properties.map((p) => {
    ...
    const isRestProperty = p.type === "ExperimentalRestProperty";
  ...
  create(context) {
    ...
    return {
      ObjectPattern(node) {
        ...
        const propertiesNum = properties.reduce((a, c) => (
          a + ((c.type === "Property" || c.type === "ExperimentalRestProperty") ? 1 : 0)
        ), 0);
        ...
        if (!blankLinesReported) {
          ...
          const expectedLineCount = propertiesNum + 2;
