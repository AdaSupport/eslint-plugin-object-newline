const fixer = (node, spacer = "\n") => (eslintFixer) => {
  ...
  const newVal = `{${spacer}${node.properties.map((p) => `${tab}${p.key.name}${p.value.name === p.key.name ? "" : `: ${p.value.name}`}`).join(`,${spacer}`)}${tab ? "," : ""}${spacer}}`;
...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url: "AdaSupport/eslint-plugin-object-newline",
    },
    fixable: "whitespace",
    schema: {
      oneOf: [
