const fixer = (context, node, spacer = "\n") => (eslintFixer) => {
  const tab = spacer === "\n" ? "  " : "";
  const type = node.typeAnnotation ? context.getSourceCode().getText(node.typeAnnotation) : "";

  const mapper = node.properties.map((p) => {
    const valName = context.getSourceCode().getText(p.value);
    const value = valName === p.key.name ? "" : `: ${valName}`;
    ...
    return {
      ObjectPattern(node) {
        ...
        if (!blankLinesReported) {
          ...
          if (singleLine) {
            const line = context.getSourceCode().getText();
